...
export default function FilmsActorList({ filmsData, isfilm }: Props) {
    const t = useTranslations('Components.genres');
    const url = isfilm ? '/films/[idname]' : '/series/[idname]';

    const sort = useSelector((state: RootState) => state.filmActorSort.sortBy);
    const direction = useSelector((state: RootState) => state.filmActorSort.direction);
